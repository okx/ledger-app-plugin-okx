...
static void handle_okx_earn_stake(ethPluginProvideParameter_t *msg, context_t *context) {
    if (context->go_to_offset) {
        if (msg->parameterOffset != context->offset + SELECTOR_SIZE) {
            PRINTF(
                "-- OKX PLUGIN ************************************* go to offset return -- %d\n",
                context->offset);
            return;
        }
            ...
            if (context->earn_calls_selector_offset != 0 &&
                ...
                if (memcmp(selector, okx_selector_deposit, SELECTOR_SIZE) == 0 ||
                    ...
                    context->operation = "Invest";
                           ...
                           memcmp(selector, okx_selector_unstake_and_withdraw, SELECTOR_SIZE) ==
                               0 ||
                ...
                if (msg->parameterOffset > PARAMETER_LENGTH * 5 + SELECTOR_SIZE &&
                    ...
                    if (current_sub_line == 1) {  // token amount
                        ...
                        if (line == 0) {
                            ...
                            copy_parameter(context->token_in_amount2,
                                           msg->parameter,
                                           sizeof(context->token_in_amount2));
                        }
                    }
                    PRINTF(
                        "-- OKX PLUGIN ************************************* TOKEN IN DATA current "
                        "line -- %d\n",
                        current_line);
                }
            }
            ...
            if (context->earn_token_out_count_offset != 0 && context->earn_token_out_count != 0) {
                ...
                if (msg->parameterOffset >
                    ...
                    if (current_sub_line == 0) {  // token contract amount
                        ...
                        if (line == 1) {
                            copy_parameter(context->token_out_amount1,
                                           msg->parameter,
                                           sizeof(context->token_out_amount1));
                        } else if (line == 2) {
                            copy_parameter(context->token_out_amount2,
                                           msg->parameter,
                                           sizeof(context->token_out_amount2));
                        }
                    }
                    PRINTF(
                        "-- OKX PLUGIN ************************************* TOKEN OUT DATA "
...
static void handle_okx_dex_unx_swap_by_order_id(ethPluginProvideParameter_t *msg,
                                                context_t *context) {
    if (context->go_to_offset) {
        if (msg->parameterOffset != context->offset + SELECTOR_SIZE) {
            PRINTF(
...
static void handle_okx_dex_uni_swapv3_swap_to(ethPluginProvideParameter_t *msg,
                                              context_t *context) {
    switch (context->next_param) {
...
static void handle_okx_dex_smart_swap_by_order_id(ethPluginProvideParameter_t *msg,
                                                  context_t *context) {
    switch (context->next_param) {
